...
import {
  resend,
  ResendRequest as AdminResendRequest,
} from "@/api/password/resendcode";
...
const VerificationCode = () => {
  ...
  const [email] = useState(
    locationState?.email || localStorage.getItem("resetEmail") || ""
  );
  const [phone] = useState(
    locationState?.phone || localStorage.getItem("resetPhone") || ""
  );
  const [phoneCountry] = useState(
    locationState?.phoneCountry ||
      localStorage.getItem("resetPhoneCountry") ||
      "EG"
  );
  ...
  return (
    <section className="flex md:flex-row flex-col items-center justify-between gap-4 !bg-white">
      ...
      <div className="px-8 mx-auto lg:mt-0 mt-10">
        ...
        <p className="text-[#7D7D7D] text-center text-lg font-normal mt-2">
        ...
        <div className="text-center mt-3 underline">
          <button
            ...
            className={`text-[19px] ${
              resendDisabled ? "text-gray-400" : "text-[#2A32F8]"
            }`}
