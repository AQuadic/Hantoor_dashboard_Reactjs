...
const RentToOwn = () => {
  ...
  const [selectedCountry, setSelectedCountry] = React.useState(getCountryByIso2("AE"));
  const [phone, setPhone] = React.useState(formData?.rent_to_own_whatsapp || "");

  // Update WhatsApp number in form when phone changes
  React.useEffect(() => {
    if (phone !== formData?.rent_to_own_whatsapp) {
      updateField?.("rent_to_own_whatsapp", phone);
    }
        ...
        <div className="mt-4">
          ...
          <div className="flex gap-4 items-start">
            <Input
              ...
              onChange={(e) => updateField?.("rent_to_own_duration", e.target.value)}
              label={t("arDuration")}
              variant="bordered"
            ...
            <Input
              ...
              onChange={(e) => updateField?.("rent_to_own_duration_en", e.target.value)}
              label={t("enDuration")}
              variant="bordered"
              placeholder={t("writeHere")}
              classNames={{ label: "mb-2 text-base" }}
            ...
            <Input
              ...
              onChange={(e) => updateField?.("rent_to_own_price", e.target.value)}
              type="number"
