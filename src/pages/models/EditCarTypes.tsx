...
import {
  updateCarType,
  UpdateCarTypePayload,
} from "@/api/models/carTypes/editCarType";
...
const EditCarTypes: React.FC = () => {
  ...
  useEffect(() => {
    if (carType) {
      ...
      setSelectedBrand(carType.brand_id?.toString() || "");
    }
  }, [carType]);
    ...
    try {
      ...
      if (err instanceof Error && "response" in err) {
        ...
        if (axiosError.response?.data?.errors) {
          ...
          Object.values(errors).forEach((msgArr: string[]) => {
            ...
            <div className="flex-1">
              ...
              >
                {(vehicleBodies?.data ?? []).map((body) => (
                  <SelectItem
                    key={body.id}
                    textValue={body.name[i18n.language as "ar" | "en"]}
                  >
