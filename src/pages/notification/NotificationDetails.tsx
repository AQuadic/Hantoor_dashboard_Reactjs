...
} from "@/api/notifications/getNotificationById";
    ...
    <section>
      ...
      <div className="md:px-8 px-2 mt-1 flex xl:flex-row flex-col gap-[33px]">
        <div className="flex-1">
          {data.image?.url ? (
            ...
            <div className="w-full h-[400px] bg-gray-100 rounded-lg flex items-center justify-center">
              <h1 className="text-[#3d3b3b] font-medium text-l">
                {t("noImage")}
              </h1>
            </div>
          )}
          ...
          <h1 className="text-[#071739] text-xl leading-6 font-bold mt-2.5">
            {data.title.ar}
          </h1>
          <p className="text-[#071739] text-base font-normal mt-[11px] xl:w-[725px] w-full">
            {data.body.ar}
          </p>
          <hr className="my-4" />
          <div className="text-left">
            <h2 className="text-[#071739] text-xl font-bold">
              {data.title.en}
            </h2>
        ...
        <div className="xl:w-[506px] w-full h-full bg-[#FFFFFF] rounded-[15px] p-[17px]">
          ...
          {/* If the notification was sent to all users show a bilingual message */}
          {data.type === "all" ? (
            <div className="mt-3">
              {i18n.language === "ar" ? (
                <p className="text-[#071739] text-base font-medium" dir="rtl">
                  تم إرسال الإشعار إلى جميع المستخدمين
                </p>
              ) : (
                <p className="text-[#606C7E] text-sm mt-2" dir="ltr">
            ...
            data.users.map((user, index) => (
              <div key={user.id ?? index} className="mt-2">
                <div className="flex items-center gap-2.5">
                  <img src="/images/user.svg" alt="" />
                  <div>
                    <h2 className="text-[#071739] text-base font-normal">
                      {user.name}
                    </h2>
                    <p
                      className="text-[#606C7E] text-sm font-normal mt-2"
                      dir="ltr"
                    >
                      {user.phone}
                    </p>
